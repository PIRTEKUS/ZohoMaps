...
- **Smart Geocoding**: Automatically converts addresses to coordinates using the Google Maps Geocoding API and caches them in the database to reduce API costs and improve performance.
...
- **Highly Scalable**: Powered by a PostgreSQL database to handle high concurrency and large volumes of data.

---

## Deployment on Ubuntu Server

The following instructions will guide you through deploying this application on a production Ubuntu server using Gunicorn, Nginx, and PostgreSQL.

### 1. Prerequisites

Ensure your Ubuntu server has Python 3, `pip`, `venv`, `nginx`, and PostgreSQL installed:

```bash
sudo apt update
sudo apt install python3-pip python3-venv python3-dev libpq-dev gcc nginx git postgresql postgresql-contrib -y
```
...
ZohoMap uses PostgreSQL for production scaling. Run the provided setup script to quickly configure a local PostgreSQL instance:
...
*(Note: The script creates a database named `zohomap` and a user named `zohouser` with password `zohopassword123!`. You can edit the script before running it if you prefer different credentials.)*
...
- `database_uri`: Set this to your PostgreSQL connection string: `postgresql://zohouser:zohopassword123!@localhost/zohomap`
...
Generate local SSL certificates for Nginx using the automated script:
```bash
./update.sh
```
...
server {
    ...
    # SSL Configuration (generated by update.sh)
    ...
    location / {
        ...
        # Crucial headers for Flask's ProxyFix
...
> 2. **config.ini**: Update the `redirect_uri` to use your new domain name (e.g., `https://yourdomain.com/callback`). After editing, run `sudo systemctl restart zohomap`.
...
---
*Note: Geocoded addresses are automatically cached in the database to save on Google Maps API calls. Subsequent map loads will be significantly faster.*
